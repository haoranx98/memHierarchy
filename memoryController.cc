...
MemController::MemController(ComponentId_t id, Params &params) : Component(id), backing_(NULL)
{
    ...
    if (!memory)
    { /* Try to load from our parameters (legacy mode 1) */
    ...
    { // Manually load via the old way of doing it
    ...
    // bool is_empty = link_ == nullptr ? true : false;
    // cout << "Get link: " << is_empty << endl;
    if (!link_ && isPortConnected("direct_link"))
    {

        // cout << "Get no link and direct_link!!!" << endl;
    ...
    MemLink *link_again = static_cast<MemLink *>(link_);
...
void MemController::handleEvent(SST::Event *event)
{
    ...
    */
    ...
    if (region_.interleaveStep != 0 && ev->getSize() > 0)
    { // Non-contiguous address region so make sure this request doesn't fall outside the region
    ...
    else if (ev->getSize() > 0)
    { // Contiguous address region, make sure last address of request falls in region
...
void MemController::adjustRegionToMemSize()
{
    ...
    if (regSize != region_.REGION_MAX)
    {              // The default is for region_.end = uint64_t -1, but then if we add one we wrap...
    ...
    if (region_.interleaveStep != 0)
    {
        ...
        if (regSize > memSize_)
        { /* Reduce the end point so that regSize is no larger than memSize */
#ifdef __SST_DEBUG_OUTPUT__
