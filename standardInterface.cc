...
void StandardInterface::send(StandardMem::Request* req) {
    ...
    // MemEvent from cpu to interface

    // 测试动态地址转换
    // MemEvent* mev1 = dynamic_cast<MemEvent*>(me);
    // if (mev1) {
    //     std::cout << "mev1 addr " << std::hex << mev1->getAddr() << std::endl;
    // }else{
    //     std::cout << "No mev1" << std::endl;
    // }
#ifdef __SST_DEBUG_OUTPUT__
...
void StandardInterface::receive(SST::Event* ev) {
    ...
    // //MemEvent 接收
    // MemEvent* mev2 = dynamic_cast<MemEvent*>(me);
    // if (mev2) {
    //     std::cout << "mev2 addr" << std::hex << mev2->getAddr() << std::endl;
    // }else{
    //     std::cout << "No mev2" << std::endl;
    // }
...
SST::Event* StandardInterface::MemEventConverter::convert(StandardMem::Read* req) {
    ...
    //测试输出
    // cout << "Dst: " << read->getDst() << endl;
    // cout << std::hex << "physical addr: " << read->getAddr() << std::endl;
    // cout << std::hex << "base addr: " << read->getBaseAddr() << std::endl;
