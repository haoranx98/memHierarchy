...
standardCPU::standardCPU(ComponentId_t id, Params &params) : Component(id), rng(id, 13)
{
    ...
    // //测试输出
    // cout << "maxAddr:" << std::hex << maxAddr << endl;
    ...
    if ((0 != ops) && (0 == (rng.generateNextUInt32() % memFreq)))
    {
        if (requests.size() < maxOutstanding)
        {
            ...
            for (int i = 0; i < reqsToSend; i++)
            {
                ...
                // if(wr_flag == "s"){
                //     cout << line.substr(8, 2) <<" write" << endl;
                // }else{
                //     cout << line.substr(8, 2) << " read" << endl;
                // }
                addr = new_addr;
...
StandardMem::Request *standardCPU::createWrite(Addr addr)
{
    ...
    // if(wr){
    //     cout << std::hex << "read addr:" << wr->pAddr << endl;
    // }
...
StandardMem::Request *standardCPU::createRead(Addr addr)
{
    ...
    // //测试指针转换
    // StandardMem::Read* rd = dynamic_cast<StandardMem::Read*>(req);
    // if(rd){
    //     cout << std::hex << "read addr:"<< rd->pAddr << endl;
    // }
...
StandardMem::Request *standardCPU::createFlush(Addr addr)
{
    ...
    // //测试指针转换
    // StandardMem::FlushAddr* fda = dynamic_cast<StandardMem::FlushAddr*>(req);
    // if(fda){
    //     cout << std::hex << "flush addr:"<< fda->pAddr << endl;
    // }
...
StandardMem::Request *standardCPU::createFlushInv(Addr addr)
{
    ...
    // StandardMem::FlushAddr* fda = dynamic_cast<StandardMem::FlushAddr*>(req);
    // if(fda){
    //     cout << std::hex << "flush inv addr:"<< fda->pAddr << endl;
    // }
