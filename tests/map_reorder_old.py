...
def plot_dual_pie_chart(old_sums, new_sums, controller_per_node, block_per_controller):
    ...
    plt.tight_layout()
    plt.savefig(f"block_distribution_{controller_per_node}_{block_per_controller}.png")
    plt.show()

def plot_single_pie_chart(sums, controller_per_node, block_per_controller):
    labels = [f"C{i}" for i in range(controller_per_node)]
    ...
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig(f"single_pie_chart_{controller_per_node}_{block_per_controller}.png")
    print("✅ 图表已保存为 block_distribution.png")
    plt.show()
    

# ✅ 参数处理
if len(sys.argv) != 4:
    print("用法: python map_reorder.py <filename> <controller_per_node> <block_per_controller>")
    sys.exit(1)
...
with open(filename, 'r') as f:
    ...
    if len(lines) > 1:
        ...
        plot_dual_pie_chart(old_sums, new_sums, controller_per_node, block_per_controller)

    else:
        old_sums = get_controller_sums(old, controller_per_node, block_per_controller)
        plot_single_pie_chart(old_sums, controller_per_node, block_per_controller)
